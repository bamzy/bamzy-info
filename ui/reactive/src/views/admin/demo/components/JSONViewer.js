...
const JSONParserViewer = () => {
  ...
  const formatJson = useCallback((_, replacer= 2) => {
    setMessage('');
    try {
      if (code) {
        const obj = JSON.parse(code);
  ...
  return (
    <div className="app">
      {!Number(param.corner) && (
        <GitHubCorners fixed zIndex={999} size={43} target="__blank" href={param.cornerhref ? param.cornerhref : ''} />
      )}
...
export default JSONParserViewer;